...
**What it does:**
1. Detect if already migrated (plans/ exists) → skip
...
7. Migrate `CURRENT_PLAN.md` → `plans/active-<username>.md`
8. Move `PENDING_REVIEW.md` → `reviews/PENDING_<username>.md` (if exists)
...
**Test scenarios:**
1. ✅ Fresh init creates multi-dev structure
2. ✅ Migration script converts existing project
3. ✅ Migration script is idempotent
4. ✅ Agents use correct file paths
5. ✅ sync-plans generates team index correctly
6. ✅ All existing tests still pass
7. ✅ No references to old single-dev pattern
...
6. Test architect review → correct review file location

---

### Phase 7: Release Preparation (20 min)

**Version bump:** v0.8.0 → v0.9.0 (minor bump, breaking change)
...
- [ ] Zero issues with single → multi-dev conversion
