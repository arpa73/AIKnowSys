...
- [ ] 2.1: Create migration helper script
  - **Action:** Write script to automate syntax conversion
  - **Why:** Consistent conversion, faster than manual
  - **Dependencies:** Phase 1 complete
  - **Risk:** Low - we can validate manually after
  - **Script Location:** `scripts/migrate-tests-to-vitest.js`
  - **Conversions:**
    - `import { test, describe } from 'node:test'` → `import { test, describe, expect, vi } from 'vitest'`
    - `import assert from 'node:assert'` → Remove (use `expect`)
    - `assert.strictEqual(a, b)` → `expect(a).toBe(b)`
    - `assert.match(str, /regex/)` → `expect(str).toMatch(/regex/)`
    - `assert.ok(value)` → `expect(value).toBeTruthy()`
    - `mock.method()` → `vi.spyOn()` or `vi.mock()`

- [ ] 2.2: Migrate test files (batch 1: core commands)
  - **Action:** Convert core command tests (init, check, migrate, etc.)
  - **Why:** Most stable, least complex tests first
  - **Dependencies:** 2.1
  - **Risk:** Medium - need to validate each conversion
  - **Files:** `test/init.test.ts`, `test/check.test.ts`, `test/migrate.test.ts`, etc.
  - **Validation:** Run `vitest` after each file, ensure passes

- [ ] 2.3: Migrate test files (batch 2: utilities)
  - **Action:** Convert utility tests (config, logger, banner, etc.)
  - **Why:** Simple tests, good for validating migration pattern
  - **Dependencies:** 2.2
  - **Risk:** Low - straightforward assertions
  - **Files:** `test/config.test.ts`, `test/logger.test.ts`, etc.

- [ ] 2.4: Migrate test files (batch 3: compress-essentials)
  - **Action:** Convert and FIX compress-essentials tests (currently skipped)
  - **Why:** Validates that mocking now works with Vitest
  - **Dependencies:** 2.3
  - **Risk:** Medium - needs new mocking approach
  - **File:** `test/compress-essentials.test.ts`
  - **Mocking Strategy:**
    ```typescript
...
- [ ] 2.5: Migrate test files (batch 4: integration tests)
  - **Action:** Convert hooks and integration tests
  - **Why:** Most complex, benefit from better module resolution
  - **Dependencies:** 2.4
  - **Risk:** Medium - complex setups
  - **Files:** `test/hooks.test.ts`, `test/quality-check.test.ts`, etc.

- [ ] 2.6: Update test helpers and fixtures
  - **Action:** Update shared test utilities for Vitest
  - **Why:** Consistent test setup across all tests
  - **Dependencies:** 2.5
  - **Risk:** Low - isolated changes
  - **Changes:** PROJECT_ROOT setup, mock helpers, etc.
...
### Phase 3: Features & Enhancements (1 hour)

**Goal:** Enable Vitest-specific features for better developer experience

**Tasks:**

- [ ] 3.1: Enable watch mode
  - **Action:** Configure watch mode for development workflow
  - **Why:** Instant feedback during test writing
  - **Dependencies:** Phase 2 complete
  - **Risk:** Low - opt-in feature
  - **Usage:** `npm run test:watch`

- [ ] 3.2: Enable coverage reporting
  - **Action:** Configure v8 coverage provider
  - **Why:** Track test coverage metrics
  - **Dependencies:** Phase 2 complete
  - **Risk:** Low - reporting only
  - **Usage:** `npm run test:coverage`
  - **Config:** Already in vitest.config.ts from Phase 1

- [ ] 3.3: Enable UI mode (optional)
  - **Action:** Test Vitest UI for visual test running
  - **Why:** Better debugging and test exploration
  - **Dependencies:** Phase 2 complete
  - **Risk:** Low - optional tool
  - **Usage:** `npm run test:ui`

- [ ] 3.4: Add CI/CD integration
  - **Action:** Update GitHub Actions workflow (if exists)
  - **Why:** Ensure tests run in CI
  - **Dependencies:** Phase 2 complete
  - **Risk:** Low - CI scripts isolated
  - **Changes:** Replace `npm run build && npm test` with `npm test`
