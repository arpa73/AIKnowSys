...
export class SqliteStorage extends StorageAdapter {
    ...
    async queryPlans(filters) {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before querying. ' +
        ...
        if (filters) {
            ...
            if (filters.topic) {
                ...
                query = query.replace('FROM plans WHERE', 'FROM plans LEFT JOIN json_each(plans.topics) WHERE');
            }
    ...
    async querySessions(filters) {
        ...
        if (filters) {
            ...
            if (filters.topic) {
                ...
                params.push(`%${filters.topic}%`, `%${filters.topic}%`);
                query = query.replace('FROM sessions WHERE', 'FROM sessions LEFT JOIN json_each(sessions.topics) WHERE');
            }
            if (filters.plan) {
                query += ' AND plan_id = ?';
        ...
        const sessions = rows.map(row => {
            ...
            if (row.phases)
                session.phases = JSON.parse(row.phases);
            return session;
        });
        return {
    ...
    async search(query, scope) {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before searching. ' +
                'Example: await storage.init(process.cwd())');
        }
        const results = [];
    ...
    async rebuildIndex() {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before rebuilding index. ' +
                'Example: await storage.init(process.cwd())');
        }
        // SQLite FTS indices are automatically maintained by triggers
        // This method returns current counts
        const plansCount = this.db.prepare('SELECT COUNT(*) as count FROM plans').get();
        const sessionsCount = this.db.prepare('SELECT COUNT(*) as count FROM sessions').get();
        const patternsCount = this.db.prepare('SELECT COUNT(*) as count FROM patterns').get();
        return {
     ...
     * @throws Error if database not initialized
     */
    async queryFullSessions(filters) {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before querying. ' +
                'Example: await storage.init(process.cwd())');
        }
     ...
     * @throws Error if database not initialized
     */
    async querySessionsMetadata(filters) {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before querying. ' +
                'Example: await storage.init(process.cwd())');
        }
     ...
     * @throws Error if database not initialized
     */
    async queryPlansMetadata(filters) {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before querying. ' +
                'Example: await storage.init(process.cwd())');
        }
    ...
    async getSessionStats(filters) {
      ...
      WHERE ${whereClause}
    `;
        const stats = this.db.prepare(statsQuery).get(...params);
        // Get status counts
        const statusQuery = `
      ...
      GROUP BY status
    `;
        const statusRows = this.db.prepare(statusQuery).all(...params);
    ...
    `;
        const topicsRows = this.db.prepare(topicsQuery).all(...params);
      ...
      LIMIT 20
    `;
        const previewRows = this.db.prepare(previewQuery).all(...params);
        ...
        return {
            ...
            uniqueTopics: Array.from(uniqueTopics).slice(0, 10), // Top 10 topics
     ...
     * @throws Error if database not initialized
     */
    async queryLearnedPatternsMetadata(filters) {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before querying. ' +
                'Example: await storage.init(process.cwd())');
        }
    ...
    async getStats() {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before querying stats. ' +
                'Example: await storage.init(process.cwd())');
        }
        const sessionCount = this.db.prepare('SELECT COUNT(*) as count FROM sessions').get();
        const planCount = this.db.prepare("SELECT COUNT(*) as count FROM plans WHERE id NOT LIKE 'learned_%'").get();
        const learnedCount = this.db.prepare("SELECT COUNT(*) as count FROM plans WHERE id LIKE 'learned_%'").get();
        return {
    ...
    async insertPlan(plan) {
        if (!this.db) {
            throw new Error('Database not initialized. Call init(targetDir) before inserting data. ' +
                'Example: await storage.init(process.cwd())');
        }
    ...
    `);
        stmt.run(plan.id, plan.project_id, plan.title, plan.status, plan.author, plan.priority || null, plan.type || null, plan.description || null, plan.content, JSON.stringify(plan.topics || []), plan.created, plan.updated);
    }
    /**
    ...
    `);
        stmt.run(session.id, session.project_id, session.date, session.topic, session.status, session.plan || null, // Map 'plan' → 'plan_id' column
        session.duration || null, session.content, JSON.stringify(session.topics || []), JSON.stringify(session.phases || []), session.created, session.updated);
    }
    /**
    ...
    async insertProject(project) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(project.id, project.name, project.path || null, project.tech_stack ? JSON.stringify(project.tech_stack) : null, project.created_at, project.updated_at);
    }
}
