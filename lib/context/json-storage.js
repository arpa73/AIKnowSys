...
export class JsonStorage extends StorageAdapter {
    ...
    async queryPlans(filters) {
        ...
        if (filters) {
            ...
            if (filters.topic) {
                plans = plans.filter(p => p.title.toLowerCase().includes(filters.topic.toLowerCase()) ||
    ...
    async querySessions(filters) {
        ...
        if (filters) {
            ...
            if (filters.topic) {
                sessions = sessions.filter(s => s.topic.toLowerCase().includes(filters.topic.toLowerCase()));
            }
            if (filters.plan) {
        ...
        for (const { dir, type } of searchDirs) {
            ...
            try {
                ...
                for (const file of files) {
                    ...
                    // Filter by type
                    if (type === 'plan' && !file.toString().startsWith('PLAN_'))
                        continue;
                    if (type === 'essentials' && file.toString() !== 'CODEBASE_ESSENTIALS.md')
                        continue;
            ...
            for (const file of learnedFiles) {
                if (file.endsWith('.md')) {
                    ...
                    if (frontmatterMatch) {
                        ...
                        const metadata = { file: `learned/${file}` };
                        lines.forEach(line => {
    ...
    parsePlanPointer(content, filename) {
    ...
    parsePlanFile(content, filename) {
        let status;
        let author;
        let created;
        let updated;
            ...
            lines.forEach(line => {
                ...
                if (key && valueParts.length > 0) {
                    ...
                    if (k === 'status')
                        ...
                        if (topicsMatch) {
                            topics = topicsMatch[1].split(',').map((t) => {
    ...
    parseSessionFile(content, filename) {
