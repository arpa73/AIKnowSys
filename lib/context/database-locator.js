...
export class DatabaseLocator {
     ...
     */
    async getDatabaseConfig(targetDir) {
     ...
     */
    async getProjectName(targetDir) {
        const resolvedDir = path.resolve(targetDir);
        const config = await this.readConfig(resolvedDir);
     ...
     */
    async getGitProjectId(targetDir) {
        try {
     ...
     */
    async ensureDatabaseDirectory(dbPath) {
        const dbDir = path.dirname(dbPath);
        await fs.mkdir(dbDir, { recursive: true });
    }
}
//# sourceMappingURL=database-locator.js.map