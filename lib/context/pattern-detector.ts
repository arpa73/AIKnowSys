...
/**
 * Detected pattern with frequency tracking
 */
...
/**
 * Options for extractErrorPatterns
 */
 ...
 * @param daysBack - How many days back to look
 * @returns Array of session objects with content and date
 */
export async function loadRecentSessions(targetDir: string, daysBack: number = 30): Promise<Session[]> {
 ...
 * @returns Array of detected patterns with frequency
 */
export function extractErrorPatterns(sessions: Session[], options: ExtractPatternOptions = {}): DetectedPattern[] {
 ...
 * @param targetDir - Project root directory
 * @param options - Optional configuration
 * @returns Patterns worth documenting
 */
export async function detectPatterns(targetDir: string, options: DetectPatternsOptions = {}): Promise<DetectedPattern[]> {
