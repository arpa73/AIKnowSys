 ...
 */
export async function validateDeliverablesCore(options = {}) {
 ...
 */
async function checkMaintainerSkillBoundary(projectRoot) {
    const issues = [];
 ...
 */
async function validateTemplateSchema(projectRoot, _options) {
    const issues = [];
    let templatesChecked = 0;
    let patternsValidated = 0;
 ...
 */
async function validatePatternConsistency(projectRoot, _options) {
    const issues = [];
    let patternsValidated = 0;
    for (const [templatePath, schema] of Object.entries(TEMPLATE_SCHEMA)) {
        if (!schema.mappedTo)
                ...
                try {
                    ...
                    for (const forbidden of schema.forbiddenPatterns || []) {
                        ...
                        if (inTemplate !== inNonTemplate) {
                            issues.push(`Pattern mismatch: "${forbidden}" ${inTemplate ? 'found in' : 'missing from'} ${templatePath} ` +
 ...
 */
async function detectLegacyPatterns(projectRoot, _options) {
    const issues = [];
    const fixable = [];
 ...
 */
async function detectUnresolvedPlaceholders(projectRoot) {
    const issues = [];
 ...
 */
async function testTemplateExecution(_projectRoot) {
    const issues = [];
 ...
 */
async function testFreshInit(_projectRoot) {
    const issues = [];
    // Stub: Returns success until Phase 2 implementation
    return {
        name: 'Fresh Init',
 ...
 */
async function autoFixPatterns(projectRoot, fixableIssues) {
    const fixes = [];
...
async function logMetrics(projectRoot, result, options) {
    ...
    try {
        ...
        if (!result.passed)
            stats.failures++;
        stats.avgDuration = Math.round((stats.avgDuration * (stats.runs - 1) + (result.metrics?.duration || 0)) / stats.runs);
...
//# sourceMappingURL=validate-deliverables.js.map