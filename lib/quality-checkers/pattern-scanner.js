 ...
 */
export async function scanPatterns(targetDir, _config = {}) {
  const violations = [];
  ...
  for (const filePath of jsFiles) {
    ...
    lines.forEach((line, index) => {
      ...
      // Check 2: require() in ES module project (exclude .cjs files and comments)
      if (isESMProject && !isCJS && /require\(/.test(line)) {
        violations.push({
          file: relativePath,
          line: index + 1,
