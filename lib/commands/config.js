...
import { loadConfig, saveConfig, isFeatureEnabled, getDefaultConfig } from '../config.js';
...
export async function enableFeature(feature, options) {
  ...
  try {
    switch (feature) {
      ...
      case 'vscodeHooks': {
        const { setupVSCodeHooks } = await import('./init/templates.js');
        await setupVSCodeHooks(targetDir, true);
        break;
      }
...
export async function uninstall(options) {
  ...
  if (!silent && !options.yes) {
    const { confirmProjectName } = await inquirer.prompt([{
      type: 'input',
