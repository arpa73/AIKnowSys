 ...
 * @param check - Check definition { name, cmd, optional }
 * @param log - Logger instance
 * @returns { passed, duration, error }
 */
async function runCheck(check: CheckDefinition, log: ReturnType<typeof createLogger>): Promise<CheckResult> {
  ...
  try {
    ...
    if (check.optional) {
      ...
      return { passed: true, duration, error: (error as Error).message, skipped: true };
    } else {
 ...
 * @returns Result with checks array
 */
export async function ciCheck(options: CiCheckOptions): Promise<{ success: boolean; checks: CiCheckResult[]; totalDuration: number }> {
  ...
  try {
    ...
    log.error(`Failed to run CI checks: ${(error as Error).message}`);
    throw error;
  } finally {
