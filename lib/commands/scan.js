...
export async function scan(options) {
  ...
  try {
    ...
    if (findings.database) console.log(chalk.gray(`   Database:       ${findings.database}`));
    if (findings.orm) console.log(chalk.gray(`   ORM:            ${findings.orm}`));
    if (findings.stateManagement) console.log(chalk.gray(`   State Mgmt:     ${findings.stateManagement}`));
    if (findings.apiClient) console.log(chalk.gray(`   API Client:     ${findings.apiClient}`));
    if (findings.authentication) console.log(chalk.gray(`   Auth:           ${findings.authentication}`));
    if (findings.styling) console.log(chalk.gray(`   Styling:        ${findings.styling}`));
