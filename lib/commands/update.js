...
export async function update(options) {
  ...
  // Ask what to update (unless --yes flag)
  let updateChoices;
  ...
  if (updateChoices.includes('agents')) {
    const agentSpinner = ora('Updating custom agents...').start();
    ...
    try {
      ...
      if (fs.existsSync(agentsDir)) {
        ...
        agentSpinner.text = 'Updating custom agents (backup created)...';
      }
      ...
      agentSpinner.succeed('Custom agents updated');
      updatedCount++;
    } catch (error) {
      agentSpinner.fail('Failed to update agents');
  ...
  if (updateChoices.includes('skills')) {
    const skillsSpinner = ora('Updating universal skills...').start();
    ...
    try {
      ...
      if (fs.existsSync(skillsDir)) {
        ...
        skillsSpinner.text = 'Updating universal skills (backup created)...';
      }
      ...
      skillsSpinner.succeed('Universal skills updated');
      updatedCount++;
    } catch (error) {
  ...
  if (updateChoices.includes('agents-md')) {
    const agentsMdSpinner = ora('Updating AGENTS.md...').start();
    ...
    try {
      ...
      if (fs.existsSync(agentsMdPath)) {
        ...
        agentsMdSpinner.text = 'Updating AGENTS.md (backup created)...';
      }
      ...
      agentsMdSpinner.succeed('AGENTS.md updated');
      updatedCount++;
    } catch (error) {
      agentsMdSpinner.fail('Failed to update AGENTS.md');
      console.log(chalk.red(`   Error: ${error.message}`));
    }
  }
  ...
  if (updateChoices.includes('essentials-md')) {
    const essentialsMdSpinner = ora('Updating CODEBASE_ESSENTIALS.md...').start();
    ...
    try {
      ...
      if (fs.existsSync(essentialsMdPath)) {
        ...
        essentialsMdSpinner.text = 'Updating CODEBASE_ESSENTIALS.md (backup created)...';
      }
      ...
      essentialsMdSpinner.succeed('CODEBASE_ESSENTIALS.md updated');
      updatedCount++;
    } catch (error) {
      essentialsMdSpinner.fail('Failed to update CODEBASE_ESSENTIALS.md');
      console.log(chalk.red(`   Error: ${error.message}`));
    }
  }
