...
export async function audit(options) {
  ...
  for (const file of filesToCheck) {
    if (fs.existsSync(file.path)) {
      ...
      if (fileSize === 0) {
        throw new Error(`${file.name} is empty or has no content. Run 'aiknowsys scan' to generate content.`);
      }
      
      // Edge case: File too large
      if (fileSize > fiftyMB) {
        throw new Error(`${file.name} is too large (${(fileSize / 1024 / 1024).toFixed(1)}MB). Consider using streaming or splitting the file.`);
      }
    }
  }
