...
export interface ListPatternsOptions {
  dir?: string;
  _silent?: boolean;
  _username?: string | null; // For testing
}
 ...
 * @returns Array of pattern objects { name, keywords }
 */
async function readPatternsFromDir(dirPath: string): Promise<Pattern[]> {
  try {
 ...
 * @returns Result object with personal and learned patterns
 */
export async function listPatterns(options: ListPatternsOptions = {}): Promise<{ success: boolean; message?: string; personal: Pattern[]; learned: Pattern[] }> {
