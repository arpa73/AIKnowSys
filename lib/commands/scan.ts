...
export async function scan(options: ScanOptions): Promise<ScanFindings> {
  ...
  const spinner: Ora | null = silent ? null : ora('Detecting project configuration...').start();
  ...
  try {
    ...
    const draftSpinner: Ora | null = silent ? null : ora('Generating draft CODEBASE_ESSENTIALS...').start();
...
function generateEssentialsDraft(findings: ScanFindings): string {
