...
interface UpdateResult {
  ...
  agentsMdHadCustomizations?: boolean;
  essentialsMdHadCustomizations?: boolean;
  alreadyUpToDate?: boolean;
  choices?: string[];
}

type UpdateChoice = 'agents' | 'skills' | 'agents-md' | 'essentials-md';
...
function saveCurrentVersion(targetDir: string, version: string): void {
  const versionFile = path.join(targetDir, '.aiknowsys-version');
