...
// @ts-ignore - scan.js not yet migrated to TypeScript, will be migrated in later batch
import { scan } from './scan.js';
// @ts-ignore - install-agents.js not yet migrated to TypeScript, will be migrated in same batch
import { installAgents } from './install-agents.js';
// @ts-ignore - install-skills.js not yet migrated to TypeScript, will be migrated in same batch
import { installSkills } from './install-skills.js';
import { getPackageDir, copyTemplate, displayAIPrompt, FileTracker } from '../utils.js';
// @ts-ignore - migrate-learned-patterns.js not yet migrated to TypeScript, will be migrated in later batch
import { migratePersonalPatterns } from '../../scripts/migrate-learned-patterns.js';
import type { MigrateOptions } from '../types/index.js';

/**
 * Migrate an existing project to use the knowledge system
 */
export async function migrate(options: MigrateOptions): Promise<void> {
  ...
  const spinner: Ora = ora('Scanning codebase...').start();
  await scan({ dir: targetDir, output: 'CODEBASE_ESSENTIALS.draft.md' });
  ...
  const { reviewed } = await inquirer.prompt<{ reviewed: boolean }>([{
    type: 'confirm',
    name: 'reviewed',
  ...
  if (!reviewed && !fs.existsSync(essentialsPath)) {
    const { action } = await inquirer.prompt<{ action: 'rename' | 'continue' | 'exit' }>([{
      type: 'rawlist',
      name: 'action',
  ...
  // Personal pattern migration (new feature)
  log.blank();
  log.cyan('══════════════════════════════════════════');
  log.header('Setting up personal pattern directory', '');
  log.cyan('══════════════════════════════════════════');
  
  await migratePersonalPatterns();
  ...
  log.dim('   • .aiknowsys/personal/<username>/');
  log.blank();
