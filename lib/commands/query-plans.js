 ...
 * ```
 */
export async function queryPlans(options = {}) {
    const log = createLogger(options._silent);
    // Validate status if provided
    if (options.status && !VALID_STATUSES.includes(options.status)) {
        const error = new Error(`Invalid plan status: ${options.status}. ` +
    ...
    try {
        ...
        if (result.count === 0) {
            ...
            if (options.status)
                log.info(`  Status: ${options.status}`);
            if (options.author)
                log.info(`  Author: ${options.author}`);
            if (options.topic)
...
//# sourceMappingURL=query-plans.js.map