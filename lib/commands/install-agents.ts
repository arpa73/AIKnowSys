...
import type { Replacements } from '../utils.js';

/**
 * Options interface for install-agents command
 */
...
/**
 * Install custom agents (Planner + Developer + Architect)
 */
export async function installAgents(options: InstallAgentsOptions): Promise<void> {
  ...
  const spinner: Ora | null = silent ? null : ora('Copying agent templates...').start();
  ...
  try {
    ...
    if (!silent && !skipPrompts) {
      const { addGuidelines } = await inquirer.prompt<{ addGuidelines: boolean }>([{
        type: 'confirm',
        name: 'addGuidelines',
