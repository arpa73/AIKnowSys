  ...
  for (const file of requiredFiles) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (fileSize === 0) {
        ...
        failed++;
        throw new Error(`${file.path} is empty or has no content. Run 'aiknowsys scan' to generate content.`);
      }
      ...
      if (fileSize > fiftyMB) {
        ...
        failed++;
        throw new Error(`${file.path} is too large (${(fileSize / 1024 / 1024).toFixed(1)}MB). Consider using streaming or splitting the file.`);
