  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (validation.valid) {
        ...
        log.log(`  ⚠ Config file invalid: ${validation.errors.join(', ')}`);
        checks.push({ name: 'Configuration', status: 'warn', error: validation.errors.join(', ') });
      }
    } catch (err) {
      log.log(`  ⚠ Config file exists but cannot be parsed: ${err.message}`);
      checks.push({ name: 'Configuration', status: 'warn', error: `Parse error: ${err.message}` });
    }
  } else {
    log.log('  ℹ No config file (using defaults)');
