...
/**
 * File tracker for atomic rollback on failure
 */
class FileTracker {
  constructor() {
    this.createdFiles = [];
    ...
    for (const file of this.createdFiles.reverse()) {
      try {
        if (fs.existsSync(file)) {
          ...
          log.dim(`   Removed: ${path.basename(file)}`);
        }
      } catch (error) {
        log.dim(`   Failed to remove: ${path.basename(file)}`);
      }
    }
      ...
      try {
        if (fs.existsSync(dir)) {
          ...
          if (contents.length === 0) {
            ...
            log.dim(`   Removed directory: ${path.basename(dir)}`);
          }
        }
      } catch (error) {
        // Ignore - directory might not be empty or already deleted
      }
    }

    log.success('Rollback complete - no partial files left behind');
    log.blank();
  }
}
...
export async function init(options) {
  ...
  try {
    ...
    await installAgentsAndSkills(targetDir, answers, silent, essentialsFile);
