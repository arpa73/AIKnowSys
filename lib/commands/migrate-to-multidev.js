 ...
 * @param {string} options.username - Override git username (for testing)
 * @returns {Promise<{success: boolean, migrated: boolean, alreadyMigrated: boolean, username: string, plansCreated: number, reviewsMigrated: boolean}>}
 */
export async function migrateToMultidev(options = {}) {
 ...
 */
function createPlansReadme(plansDir) {
 ...
 * @param {string} reviewsDir - Reviews directory path
 */
function createReviewsReadme(reviewsDir) {
 ...
 * @param {string} content - Old CURRENT_PLAN.md content
 * @param {string} username - Developer username
 * @returns {string} Transformed content for active-<username>.md
 */
function transformToPlanPointer(content, username) {
 ...
 * @param {string} username - Developer username
 * @returns {string} Default plan pointer content
 */
function createDefaultPlanPointer(username) {
  const now = new Date().toLocaleString('en-US', {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
 ...
 */
function updateGitignore(gitignorePath, log) {
  let content = '';
