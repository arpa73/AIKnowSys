 ...
 * @param {string} essentialsFile - Custom essentials filename (default: CODEBASE_ESSENTIALS.md)
 * @returns {Promise<void>}
 */
export async function createKnowledgeSystemFiles(targetDir, answers, templateType, essentialsFile = 'CODEBASE_ESSENTIALS.md') {
 ...
 * @param {string} targetDir - Absolute path to target directory
 * @param {Object} answers - User answers including useTDD
 * @param {boolean} silent - Whether to suppress spinner output
 * @param {string} essentialsFile - Name of essentials file
 * @param {Object} options - Optional selective installation { agents: boolean, skills: boolean }
 * @returns {Promise<void>}
 */
export async function installAgentsAndSkills(targetDir, answers, silent = false, essentialsFile = 'CODEBASE_ESSENTIALS.md', options = {}) {
  const { agents = true, skills = true } = options;
  
  if (agents) {
    const agentSpinner = silent ? null : ora('Installing custom agents...').start();
  ...
  if (skills) {
    const skillsSpinner = silent ? null : ora('Installing universal skills...').start();
  ...
  if (skills) {
    const mappingSpinner = silent ? null : ora('Generating skill mapping...').start();
...
function getGitUsername() {
  try {
    // Try local first, fallback to global for CI environments
    let username;
    try {
      username = execSync('git config user.name', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] }).trim();
    } catch {
      // Fallback to global config (CI, fresh repos)
      username = execSync('git config --global user.name', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] }).trim();
    }
 ...
 * @returns {Promise<void>}
 */
export async function setupSessionPersistence(targetDir, silent = false) {
  const aiknowsysSpinner = silent ? null : ora('Setting up session persistence...').start();
  const packageDir = getPackageDir();
 ...
 * @returns {Promise<void>}
 */
export async function setupTDDEnforcement(targetDir, silent = false) {
  const tddSpinner = silent ? null : ora('Setting up TDD enforcement...').start();
  const packageDir = getPackageDir();
...
/**
 * Setup VSCode hooks for automated session tracking
 * @param {string} targetDir - Absolute path to target directory
 * @param {boolean} silent - Whether to suppress spinner output
 * @returns {Promise<void>}
 */
export async function setupVSCodeHooks(targetDir, silent = false) {
  ...
  if (hooksSpinner) hooksSpinner.succeed('VSCode hooks installed (10 files, 8 hook types)');
}
