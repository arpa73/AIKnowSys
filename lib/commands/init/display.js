 ...
 */
export async function displayAIBootstrapPrompt(projectName, hasCode, useOpenSpec = false) {
  const log = createLogger(false);
  ...
  const copiedToClipboard = await displayAIPrompt(log, promptLines);
  ...
  // Show clipboard status as final message (most visible!)
