 ...
 * @param targetDir - Directory to initialize OpenSpec in
 * @param silent - Whether to suppress spinner output
 * @returns Whether setup was successful
 */
export async function setupOpenSpec(targetDir: string, silent = false): Promise<boolean> {
  const openSpecSpinner: Ora | null = silent ? null : ora('Setting up OpenSpec...').start();
