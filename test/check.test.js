...
`);
  ...
  it('should detect bloated ESSENTIALS (>800 lines)', () => {
    ...
    const essentialsPath = path.join(testDir, 'CODEBASE_ESSENTIALS.md');
    const bloatedContent = `# Codebase Essentials
...
`;
  ...
  it('should detect verbose sections (>150 lines)', () => {
    ...
    const essentialsPath = path.join(testDir, 'CODEBASE_ESSENTIALS.md');
    const verboseSection = `# Codebase Essentials
...
`;
  ...
  it('should not warn for reasonably-sized ESSENTIALS (<800 lines)', () => {
    ...
    const essentialsPath = path.join(testDir, 'CODEBASE_ESSENTIALS.md');
    const reasonableContent = `# Codebase Essentials
...
`;
    ...
    const essentialsLines = lines.filter(l => l.includes('ESSENTIALS') || l.includes('Essentials'));
    
    // The ESSENTIALS line should show a checkmark, not a warning
    const hasEssentialsWarning = essentialsLines.some(l => 
