...
describe('list-patterns command', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(__dirname, 'fixtures', 'list-patterns-test');
  ...
  it('should list personal patterns', async () => {
    ...
    const personalDir = path.join(testDir, '.aiknowsys', 'personal', 'test-user');
    ...
    const result = await listPatterns({ dir: testDir, _silent: true, _username: 'test-user' });

    assert.strictEqual(result.success, true);
    assert.strictEqual(result.personal.length, 2);
    assert.ok(result.personal.some(p => p.name === 'api-retry.md'));
    assert.ok(result.personal.some(p => p.name === 'vue-composable.md'));
  });
  ...
  it('should list team patterns from learned directory', async () => {
    ...
    assert.ok(result.learned.some(p => p.name === 'database-pooling.md'));
    assert.ok(result.learned.some(p => p.name === 'error-handling.md'));
  });
  ...
  it('should error if no git username available', async () => {
    const result = await listPatterns({ dir: testDir, _silent: true, _username: null });
