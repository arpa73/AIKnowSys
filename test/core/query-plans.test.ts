...
describe('queryPlansCore (Pure Business Logic)', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.AIKNOWSYS_DB_PATH = resolve(TEST_DIR, '.aiknowsys', 'knowledge.db');
...
`;
  ...
  describe('Basic Queries', () => {
    it('should return empty results when no plans exist', async () => {
      const result = await queryPlansCore({ adapter: 'sqlite', }, TEST_DIR);
    ...
    it('should filter by status: ACTIVE', async () => {
      ...
      const result = await queryPlansCore({ adapter: 'sqlite', status: 'ACTIVE' }, TEST_DIR);
    ...
    it('should filter by author', async () => {
      ...
      const result = await queryPlansCore({ adapter: 'sqlite', author: 'alice' }, TEST_DIR);
    ...
    it('should filter by topic (fuzzy match)', async () => {
      ...
      const result = await queryPlansCore({ adapter: 'sqlite', topic: 'react' }, TEST_DIR);
  ...
  describe('Date Range Filtering', () => {
    it('should filter by updatedAfter', async () => {
      ...
      const result = await queryPlansCore({ adapter: 'sqlite', updatedAfter: '2026-02-01' }, TEST_DIR);
    ...
    it('should filter by updatedBefore', async () => {
      ...
      const result = await queryPlansCore({ adapter: 'sqlite', updatedBefore: '2026-02-01' }, TEST_DIR);
  ...
  describe('Purity & Structure', () => {
    it('should NOT log anything (pure function)', async () => {
      ...
      await queryPlansCore({ adapter: 'sqlite', }, TEST_DIR);
